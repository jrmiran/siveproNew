...
export class NewBudgetV2Component implements OnInit {
    ...
    changeAmbient(a: string){
        ...
        var oldAmbient = this.itemsBudget.find((v) => {return v == this.selectedItemBudget}).ambient;
        
        
        this.itemByAmbient.find((v) => {return v.ambient == oldAmbient}).items = this.itemByAmbient.find((v) => {return v.ambient == oldAmbient}).items.filter((d) =>{return d != this.selectedItemBudget});
        this.itemByAmbient.find((v) => {return v.ambient == oldAmbient}).ambientValue = this.itemByAmbient.find((v) => {return v.ambient == oldAmbient}).ambientValue - this.selectedItemBudget.totalValue;
        
        
        this.selectedItemBudget.ambient = a;
        this.itemByAmbient.find((v) => {return v.ambient == a}).items.push(this.selectedItemBudget);
        this.itemByAmbient.find((v) => {return v.ambient == a}).ambientValue = this.itemByAmbient.find((v) => {return v.ambient == a}).ambientValue + this.selectedItemBudget.totalValue;
    ...
    // ------------------------------------------------------------------------------------------------
    // FUNÇÃO DO BOTÃO QUE TROCA AMBIENTE DE POSIÇÃO --------------------------------------------------
    changeAmbientPosition(type: string, index: number){
        console.log(index + " - " + this.ambients.length);
        if(type == 'up' && index > 0){
            this.ambients = this.array_move(this.ambients, index, index-1);
        } else if(type == 'down' && index < this.ambients.length-1){
            this.ambients = this.array_move(this.ambients, index, index+1);
        }
        
        var itemsByAmbient: ItemByAmbient[] = [];
        this.ambients.forEach((data)=>{
            itemsByAmbient.push(this.itemByAmbient.find((v)=>{return v.ambient == data}));
        })
        itemsByAmbient = itemsByAmbient.filter((v)=>{return v != undefined})
        this.itemByAmbient = itemsByAmbient;
    ...
    assignItemsBudget(){
        ...
        this.itemsBudgetParam.sort((a,b) => a['numero'] < b['numero'] ? -1 : 1);
        this.itemsBudgetParam.forEach((v)=>{
            ...
            /*if(item.item != "LINHA DE SEPARAÇÃO"){
               if(this.ambients.indexOf(item.ambient) < 0){
                   this.ambients.push(item.ambient);
               }
           } */
        });
    ...
    // FUNÇÃO DO BOTÃO 'UP' E 'DOWN' PARA ALTERAR POSIÇÃO DO ITEM -------------------------------------
    changeIndexItem(data: any, type: string){
        var index: number = this.itemsBudget.indexOf(data);
        var ambient: string = data.ambient;
        var ambientIndex: number = this.itemByAmbient.find((v)=>{return v.ambient == ambient}).items.indexOf(data);

        if(type == 'up' && index > 0){
            if(this.itemsBudget[index-1].item != "LINHA DE SEPARAÇÃO"){
                this.array_move(this.itemsBudget, index, index-1);
                this.itemsBudget[index].number = this.itemsBudget[index].number +1;
                this.itemsBudget[index-1].number = this.itemsBudget[index].number -1;
                this.array_move(this.itemByAmbient.find((v)=>{return v.ambient == ambient}).items, ambientIndex, ambientIndex-1);
            }
        } else if(type == 'down' && index < this.itemsBudget.length-1){
            if(this.itemsBudget[index+1].item != "LINHA DE SEPARAÇÃO"){
                this.array_move(this.itemsBudget, index, index+1);
                this.itemsBudget[index].number = this.itemsBudget[index].number -1;
                this.itemsBudget[index+1].number = this.itemsBudget[index].number +1;
