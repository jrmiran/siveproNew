...
import {soExecution} from '../service-order/so-execution.model';
...
@Component({
  selector: 'sivp-order-service-test',
  templateUrl: './order-service-test.component.html',
  styleUrls: ['./order-service-test.component.css']
})
export class OrderServiceTestComponent implements OnInit {
       ...
       //self.checkBoxFormArray = new FormArray([]);
        ...
        this.checks.forEach(function(data, index){
           self.checkBoxFormArray.push(new FormGroup({
               ...
               employeeEmpreitaShare: new FormControl("0", []),
               employeeNormalShare: new FormControl("100", []),
           }))     
        })
    }
